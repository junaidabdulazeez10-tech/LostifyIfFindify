    ...
    <>
      <nav className={`navbar border mt-3 ${mode ? "border-light bg-dark text-light" : "border-dark light-background  text-dark"}`}>
        <div className="container-fluid d-flex flex-wrap align-items-center justify-content-between gap-2">
          <button onClick={() => setMode(prevMode => !prevMode)} className={`${mode ? "btn btn-outline-light" : "btn btn-outline-dark"}`} type='button'>
          ...
          {token ?
            ...
            :
            <div className="d-flex flex-column gap-2">
              <button className="btn btn-outline-success" type='button' data-bs-toggle="modal" data-bs-target="#sign-up-form">Sign up</button>
              <button className="btn btn-outline-success" type='button' data-bs-toggle="modal" data-bs-target="#log-in-form">Log in</button>
            </div>
          }
        </div>
      </nav>
